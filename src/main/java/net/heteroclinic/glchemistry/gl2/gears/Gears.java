...
public void display(GLAutoDrawable drawable) {
  ...
  public static boolean verticesGenerated = false;
  
  public static void generateVertices(GL2 gl, float inner_radius,
			float outer_radius, float width, int teeth, float tooth_depth) {
	  if (verticesGenerated == true)
		  return;
		int i;
		float r0, r1, r2;
		float angle, da;
		...
		for (i = 0; i < teeth; i++) {
			...
			gl.glVertex3f(r2 * (float) Math.cos(angle + da),
					r2 * (float) Math.sin(angle + da), width * 0.5f);
